...
asynchronous operations and parallelisation. Therefore, the function may be
invoked only after persistent background processes are set up. This can be
done with the `daemons()` function, which allows to control the number of
processes to use; refer to the original `mirai` documentation for a complete
...
relying on deparsing of the input, which may be poor. A convenient way of
generating names is by using the `namify()` helper function.
