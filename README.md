...
To fuzz the first three arguments of `matrix()` (`data`, `nrow` and
...
In order to reduce the number of false positive results produced,
...
- If the error or warning message contains any of the patterns specified
  in the `ignore_patterns` argument. In case of multiple warnings being
...
Note that, like in one of the examples above, whitelisting can also be
...
generated by `test_inputs()`) and `args`. In such cases, `args` is
expanded internally into multiple lists, each differing by one element
modified according to an element in `what`.
...
If all elements in `args` are fixed, `what` is ignored and all functions
in `funs` will be called with the provided `args` list. A similar
behaviour can be obtained by specifying no fixed arguments in `args`
while setting `what = NULL`.
...
2.  Manually setting up the daemons before the start of the function:
    ...
    multiple times. It remains responsibility of the user to close the
    daemons when no longer in use. When active daemons are found, the
    `daemons` argument will be ignored. Refer to the original `mirai`
...
528704761](https://gepris.dfg.de/project/528704761) led by Dr Sebastian
Kreutzer (PI at LIAG - Institute for Applied Geophysics, Hannover, DE)
and Dr Thomas Kolb (PI at Justus-Liebig-University Giessen, DE). Updates
on the REPLAY project at large are available at the [REPLAY
website](https://replay.geog.uni-heidelberg.de/).
