...
#'
#' @param package Name of the package where functions are searched.
...
#' \item{runs}{a data frame containing the following columns:\cr
#'       - `idx`: The index of the input tested.\cr
...
#'       - `what`: The character representation of the input tested.\cr
#' }
...
#'
#' The `res` column in the `$runs` data frame can contain the following values:
...
fuzz <- function(funs, what = test_inputs(), args = NULL,
  ...
  ## assign display labels to `what` and `args` from the quoted expressions
  if (!is.null(what))
  ...
  not.found <- grep("Object not found in the .* namespace",
                    runs$msg[runs$idx == 1])
...
whitelist <- function(object, patterns) {
  ...
  object$runs$res[grepl(joined_patterns, object$runs$msg) &
                  object$runs$res != "SKIP"] <- "OK"
